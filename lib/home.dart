...
class MyHomePage extends StatelessWidget {
  ...
  _launchURLApp() async {
    var url = Uri.parse("https://github.com/pcm-dpc/COVID-19");
    await _launch(url);
  }

//valid for vaccini and Immuni
  _launchURLCommisssarioApp() async {
    var url = Uri.parse('https://github.com/italia/covid19-opendata-vaccini');
    await _launch(url);
  }

  _launchURLMinSaluteApp() async {
    var url = Uri.parse('https://github.com/ministero-salute');
    await _launch(url);
  }
              ...
              builder: (context, snapshot) {
                if (snapshot.hasData) {
                  return Column(
                      ...
                      children: [
                        Text.rich(
                            TextSpan(text: 'Vaccinati: ', children: <TextSpan>[
                          TextSpan(
                              text: NumberFormat.compact(locale: "it_IT")
                                      .format(snapshot.data?.personeVaccinate) +
                        ...
                        Text.rich(TextSpan(
                            ...
                            children: <TextSpan>[
                              TextSpan(
                                  text: NumberFormat.compact(locale: "it_IT")
                                      .format(snapshot.data?.dosiSomministrate),
                                  style: TextStyle(
                                      fontWeight: FontWeight.bold,
                                      color: CupertinoColors.systemPurple,
                                      fontSize: 18))
